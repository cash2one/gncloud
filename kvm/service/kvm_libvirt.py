...
def kvm_create(name, cpu, memory, disk, base_name, base_sub_type, host_ip):
    ...
    if base_sub_type == "base":
        s.sendline("\cp "+config.LIVERT_IMAGE_BASE_PATH+base_name +" "+config.LIVERT_IMAGE_LOCAL_PATH+name)
    else:
        s.sendline("\cp "+config.LIVERT_IMAGE_SNAPSHOT_PATH+base_name +" "+config.LIVERT_IMAGE_LOCAL_PATH+name)
    s.logout()
