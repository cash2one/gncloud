...
from kvm.db.models import GnGuestMachine
...
def server_create(name, cpu, memory, hdd):
    try:
        ...
        guest_machine = GnGuestMachine(name=name,cpu=cpu,memory=memory,hdd=hdd,kind='kvm',ip=ip)
...
def server_list():
    list = GnGuestMachine.query.all();
    return list

def server_change_status(name, status):
    kvm_change_status(name, status);
    if status == 'delete':
        db_session.query(GnGuestMachine).filter(GnGuestMachine.name == name).delete();
        db_session.commit()
