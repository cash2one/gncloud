    ...
    def invoice_calc(self):
        ...
        for stat in status:
            ...
            if team.team_code != team_code and team_code is not None:
                all_text = "{'버전':'%s', '년도':'%s', '과금월':'%s', '발급일':'%s', '팀아이디':'%s', '팀총가격':'%d', '사용자별 리스트':[%s] }" \
