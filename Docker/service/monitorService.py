...
def service_monitoring(sql_session):
    try:
        ...
        net_usage = 0.0
        worker_count = 0
        container_list = sql_session.query(GnDockerContainers).all()
        for container in container_list:
            ...
            docker_stat_cmd = 'docker -H %s:%s stats --all --no-stream' % (ip, port)
            ...
            vm_monitor_hist = GnMonitorHist(
                id=container.service_id, type="docker", cpu_usage=cpu_usage, mem_usage=mem_usage,
